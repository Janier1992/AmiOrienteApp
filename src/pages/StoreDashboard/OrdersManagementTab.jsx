...
const OrdersTab = ({ storeId, terminology = {} }) => {
  ...
  const T = {
    order: terminology.order || 'Pedido'
  };

  const handleDelete = async (orderId) => {
    if (!window.confirm(`¿Estás seguro de que deseas eliminar este ${T.order.toLowerCase()}? Esta acción no se puede deshacer.`)) return;
    try {
      await deleteOrder(orderId);
      toast({ title: `${T.order} eliminado` });
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      ...
      toast({ title: "Estado Actualizado", description: `El ${T.order.toLowerCase()} ahora está: ${newStatus}` });
  ...
  if (isLoadingOrders && orders.length === 0) return <LoadingSpinner text={`Cargando ${T.order.toLowerCase()}s...`} />;

  return (
    <Card className="animate-in fade-in duration-300">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Gestión de {T.order}s</CardTitle>
      ...
      <CardContent>
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-2 bg-slate-100 font-semibold text-sm text-slate-600 rounded-t-lg">
              <div className="col-span-2">ID {T.order}</div>
              <div className="col-span-3">Cliente</div>
              <div className="col-span-2">Fecha</div>
              <div className="col-span-2">Total / Items</div>
              <div className="col-span-3 text-center">Estado / Acciones</div>
            </div>
              ...
              {orders.map((order) => {
                ...
                try {
                  if (order.delivery_address?.startsWith('{')) {
                    ...
                    if (parsed.guest?.name) customerName = parsed.guest.name;
                  }
                } catch (e) { }
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                          <span className="font-mono font-bold text-slate-700">#{order.id.substring(0, 8)}</span>
                      ...
                      <div className="border-t border-b py-2">
                        ...
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-500">{order.order_items?.length || 0} ítems</span>
                          <span className="font-bold text-lg text-slate-800">${Number(order.total).toLocaleString()}</span>
                        </div>
                      </div>

                      <div className="flex justify-end gap-2">
                        <Button variant="outline" size="sm" className="h-8 flex-1" onClick={() => openDetails(order)}>
                          Detalles
                        </Button>

                        {['Pendiente', 'Pendiente de pago en efectivo'].includes(order.status) && (
                          <Button size="sm" className="h-8 bg-blue-600 flex-1" onClick={() => handleUpdateStatus(order.id, 'Confirmado')}>Confirmar</Button>
                        )}
                        {order.status === 'Confirmado' && (
                          <Button size="sm" className="h-8 bg-purple-600 flex-1" onClick={() => handleUpdateStatus(order.id, 'En preparación')}>Preparar</Button>
                        )}
                        {order.status === 'En preparación' && (
                          <Button size="sm" className="h-8 bg-indigo-600 flex-1" onClick={() => handleUpdateStatus(order.id, 'Listo para recogida')}>
                            Listo
                          </Button>
                        )}
                      </div>
            ...
            <div className="hidden md:block space-y-2">
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="border rounded-lg p-3 grid grid-cols-12 gap-4 items-center bg-white hover:bg-slate-50 transition-colors shadow-sm">
                    {/* ID */}
        ...
        <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{T.order} #{selectedOrder?.id.substring(0, 8)}</DialogTitle>
              <DialogDescription>
              ...
              {selectedOrder?.status !== 'Cancelado' && selectedOrder?.status !== 'Entregado' && (
                <Select onValueChange={(val) => handleUpdateStatus(selectedOrder.id, val)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Cancelado">Cancelar {T.order}</SelectItem>
                  </SelectContent>
                </Select>
              )}
            </div>
