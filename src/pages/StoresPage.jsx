...
import { Search, MapPin, Star, Filter, ShoppingBag, Clock, Phone, MessageCircle } from 'lucide-react';
  ...
  >
    <Card className="h-full flex flex-col hover:shadow-xl transition-all duration-300 group overflow-hidden border-slate-200">
      ...
      <CardFooter className="p-4 pt-0 mt-auto border-t border-slate-100 bg-slate-50/50">
        ...
        {store.phone && (
          <Button
            className="w-full mt-2 bg-green-600 hover:bg-green-700 text-white border-none"
            onClick={() => window.open(`https://wa.me/57${store.phone.replace(/\D/g, '')}?text=Hola, vi tu negocio ${store.name} en MiOriente y quisiera más información.`, '_blank')}
          >
